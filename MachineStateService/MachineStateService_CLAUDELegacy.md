...
- **Target (ADR-001, Proposed)**: `machinedb` → `machine_db` on MongoDB
  Atlas (P3) — `machines`, `cycles`, `reservations` as regular collections;
  `machine_events` and `telemetry` as **time-series collections** with TTL
  indexes. This service becomes the **consumer side of P4's outbox
  pattern**: subscribes to `PaymentSucceeded` via Change Streams, consumes
  idempotently via an `idempotency_keys` collection (TTL-indexed, keyed on
  `transactionReference`), then publishes `CycleCompleted` for the Reporting
  BFF (P5).
- Of the three services, this one has the **strongest case for the document
  model** (telemetry/events are naturally document/time-series shaped) —
  if root CLAUDE.md's "Open Disagreement" about P3 gets revisited, this
  service's data is the part of the argument that actually favors Mongo.
  `machines`/`cycles`/`reservations` are more relational (reservation →
  machine FK) but smaller/lower-volume than telemetry, so the trade-off
  here is less clear-cut than in PaymentManagementService.
