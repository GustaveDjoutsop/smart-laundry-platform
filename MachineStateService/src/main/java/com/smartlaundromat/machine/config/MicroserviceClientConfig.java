...
/**
 * Configures the shared Auth0-M2M-authenticated RestTemplate used for outbound
 * calls to spring-bot-manager-only's {@code POST /api/notifications/send}
 * (Auth0 scope {@code sls-bot-admin}), used by {@link
 * com.smartlaundromat.machine.service.notification.BotNotificationClient} to
 * push the pre-slot reservation reminder. Mirrors PaymentManagementService's
 * {@code MicroserviceClientConfig} exactly, reusing the same {@code
 * smartlaundry-m2m} Auth0 application already granted this scope elsewhere.
 *
...
public class MicroserviceClientConfig {
    ...
    @Bean("botManagerRestTemplate")
    @ConditionalOnExpression("'${spring.security.oauth2.client.registration.smartlaundry-m2m.client-secret:}' != ''")
    public RestTemplate botManagerRestTemplate(OAuth2AuthorizedClientManager microserviceAuthorizedClientManager) {
        log.info("BotManagerService RestTemplate configured with OAuth2 client credentials ({})", registrationId);
     ...
     * BotManagerService unauthenticated. {@link
     * com.smartlaundromat.machine.service.notification.BotNotificationClient}
     * already wraps its call in a try/catch and logs+skips on failure.
     */
    @Bean("botManagerRestTemplate")
    @ConditionalOnExpression("'${spring.security.oauth2.client.registration.smartlaundry-m2m.client-secret:}' == ''")
    public RestTemplate botManagerRestTemplateFallback() {
        log.warn("Auth0 M2M client 'smartlaundry-m2m' is not configured (missing client-secret) — "
                + "calls to BotManagerService will fail closed.");
        RestTemplate restTemplate = new RestTemplate(httpRequestFactory());
        restTemplate.getInterceptors().add((request, body, execution) -> {
