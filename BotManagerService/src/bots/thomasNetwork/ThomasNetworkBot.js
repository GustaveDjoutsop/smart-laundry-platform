...
class ThomasNetworkBot extends BaseBot {
  constructor(config) {
    super(config);

    this.flowEngine = new FlowEngine({ botConfig: config, plugin: new ThomasNetworkFlowPlugin({ botConfig: config }) });
  ...
  async handleMessage({ from, message }) {
    ...
    const conversationKey = buildConversationKey({ botId, customerPhone: from });
    ...
    const sendIntent = async (outboundIntent) => {
      ...
      if (outboundIntent.type === 'buttons') {
        await this.whatsapp.sendButtons({ to: outboundIntent.to, body: outboundIntent.body, buttons: outboundIntent.buttons });
        return;
      }
    ...
    const result = await this.flowEngine.step({ from, message, state: conversationState, send: sendIntent });

    await redisManager.setex(conversationKey, appConfig.redis.ttlSeconds, JSON.stringify(result.state));

    logger.info(
      `ThomasNetworkBot handled message from ${from} (flow=${result.state.currentFlowId}, state=${result.state.currentStateId})`
    );
  }
