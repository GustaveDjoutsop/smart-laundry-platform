  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (mode === 'create') {
      const payload: BotConfigRequest = {
        ...
        campaySecret: form.campaySecret || undefined,
        campayToken: form.campayToken || undefined,
        config,
      ...
      <section className="rounded-xl border border-gray-200 bg-white shadow-sm">
        ...
        <div className="grid grid-cols-1 gap-5 p-6 sm:grid-cols-2">
          <InputField
            ...
            type="password"
            value={form.campaySecret}
            onChange={(v) => setField('campaySecret', v)}
            placeholder={
              mode === 'edit' && bot?.hasCampaySecret ? '(unchanged)' : 'Enter App Webhook Key'
            }
            hint={'Called "App webhook key" in your CamPay web account. Used to verify incoming webhook signatures.'}
          />
          <InputField
            id="campayToken"
            label="CamPay Token (Permanent Access Token)"
            type="password"
            value={form.campayToken}
            onChange={(v) => setField('campayToken', v)}
            placeholder={
              mode === 'edit' && bot?.hasCampayToken ? '(unchanged)' : 'Enter Permanent Access Token'
            }
            hint={'Called "Permanent Access Token" in your CamPay web account. Used to authenticate API calls.'}
          />
        </div>
      </section>
