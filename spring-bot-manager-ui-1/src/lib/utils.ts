...
export function formatRelativeTime(epochSeconds: number): string {
  try {
    return formatDistanceToNow(new Date(epochSeconds * 1000), { addSuffix: true });
  } catch {
...
/**
 * Format an epoch-seconds timestamp to a locale date/time string
 */
...
/**
 * Format an epoch-seconds timestamp to a short date
 */
export function formatDate(epochSeconds: number): string {
  try {
    return format(new Date(epochSeconds * 1000), 'dd MMM yyyy');
  } catch {
    return String(epochSeconds);
  }
}
