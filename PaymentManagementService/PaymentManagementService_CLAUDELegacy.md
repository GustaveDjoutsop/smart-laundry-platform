...
- This service uses `ddl-auto: update` (Hibernate), not Flyway —
  `paymentdb` schema drift risk in the meantime.
...
- **Target (ADR-001, Proposed)**: `paymentdb` → `payment_db` on MongoDB
  Atlas (P3) — `transactions`, `rfid_cards`, `topups`, plus new `outbox` and
  `idempotency_keys` (TTL-indexed) collections for P4. Debit + transaction
  insert becomes a multi-document Mongo transaction with `writeConcern:
  majority`. **P4's outbox pattern is the actual fix for the W4/W5
  reliability gap** — once live, `MachineStartService`'s synchronous call
  is removed entirely, replaced by `PaymentSucceeded` → Change Streams →
  MachineStateService consumes idempotently. Don't build new features on
  top of the current synchronous call expecting it to survive P4 unchanged.
- If P3 (Mongo migration) is reconsidered per root CLAUDE.md's "Open
  Disagreement," this service is the one where the relational-vs-document
  trade-off is sharpest — `transactions`/`rfid_cards`/`topups` are exactly
  the kind of data Option A3 (Postgres schema-per-service) handles natively
  via FKs, and where Mongo's "ACID via multi-document transactions" is
...
write a `PaymentSucceeded` record to an `outbox` collection in the same
transaction as marking `SUCCESSFUL`, relay via MongoDB Change Streams,
MachineStateService consumes idempotently keyed on `transactionReference`.
On permanent failure, emit `MachineStartFailed` for refund/staff alert (Saga
compensation).

**If asked to "improve payment reliability" before P3 (Mongo migration) has
landed**: don't build a parallel ad-hoc retry mechanism — that's exactly the
kind of one-off fix P4 is meant to replace, and building it twice wastes
effort. The interim fix is the P0 auth fix above (so the call at least has a
chance of succeeding); full reliability is P4's job and depends on P3's
transaction/outbox infrastructure. If the business genuinely can't wait for
P3→P4, say so explicitly — that's a sequencing decision (pull P4's pattern
forward onto Postgres before the Mongo migration), not a "just add it" task.
